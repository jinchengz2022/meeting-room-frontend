...
export const getCaptchaCode = async (adress: string) => {
  return await axiosInstance.get(`/email/getCode?address=${adress}`);
};

export const getImgUrl = async (name: string) => {
  return await axiosInstance.get(`/minio/presignedUrl?name=${name}`);
};
